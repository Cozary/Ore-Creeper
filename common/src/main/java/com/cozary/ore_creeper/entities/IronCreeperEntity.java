...
public class IronCreeperEntity extends AbstractOreCreeperEntity {
    ...
    public static boolean canOreCreeperSpawn(EntityType<? extends AbstractOreCreeperEntity> creeper, ServerLevelAccessor world, EntitySpawnReason reason, BlockPos pos, RandomSource random) {
        return pos.getY() < CommonConfigManager.getConfig().ironCreeperMaxSpawnYLevel() && world.getBlockState(pos.below()).is(ModTags.SPAWNABLE_BLOCKS) && isDarkEnoughToSpawn(world, pos, random) && checkMobSpawnRules(creeper, world, reason, pos, random);
    }
    ...
    public void explodeCreeper() {
        ...
        if (!this.level().isClientSide()) {
            ...
            ((ServerLevel) this.level()).sendParticles(ParticleList.IRON_EXPLOSION.get(), this.getX() + 0.5, this.getY(), this.getZ() + 0.5, 500, d1, d2, d0, 0.5);
            this.discard();
